...
- **One tap target — the seal — drives a 4-beat sequence**, each beat timed by its own `setTimeout` in `handleOpen`, deliberately unhurried so it reads as a hand actually opening an envelope rather than a UI transition:
  1. **Open**: the flap starts tilting up immediately (`rotateX(150deg)`, 900ms, `.envelope-box__flap--open`, seal riding along with it — uncounter-rotated, fading out as it turns away, see above); the card waits a beat (`CARD_RISE_DELAY`, 300ms, a CSS `transition-delay` on `.envelope-box__card`) before it lifts out at a slight angle and straightens as it settles (`translateY` + `scale` + `rotate(-7deg → 0)`, 900ms, `.envelope-box__card--risen`) — the stagger is what sells "pulled out from behind the flap" instead of both pieces moving in lockstep
