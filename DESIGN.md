...
- **Ported from a separate prototype** (`InteractiveWeddingInvite`'s `Envelope.tsx`, built in React + Tailwind + Framer Motion) and rebuilt here in this project's plain JS/CSS style — no new dependencies added, and the palette was swapped from that prototype's rose/charcoal to this site's own tokens (`--color-accent` gold seal, `--color-bg` ivory card) so it reads as part of the same site rather than a mismatched import
- **Dark scene, no photo asset**: unlike the rest of the site (which dropped its one background photo, see Hero above), this scene is intentionally the one dark moment on the whole site — a charcoal gradient backdrop (`--envelope-charcoal-1/2/3`) standing in for the ported design's blurred background photo, plus the same inline SVG `feTurbulence` grain trick used elsewhere on the site for paper texture (`--envelope-noise`) rather than a real image
- Structure, all siblings inside `.envelope-intro__content`: a small **Ganesh crest** (shared `GaneshArt.jsx`, same artwork as Hero and the favicon) → the tagline as an overline → the **envelope box** (`.envelope-box`: a dark gradient body, an SVG triangle flap on top, an ivory "peeking" card of the couple's names + wedding date tucked above the flap, and a circular gold monogram seal, e.g. "M & Y", overlapping the flap's tip) → a "tap to open" hint with a bouncing arrow that fades in after a 3.2s pause (`HINT_DELAY`), so it doesn't compete with the initial view
- **Two independent tap targets, either opens it**: both the peeking card and the seal are separate buttons calling the same `handleOpen` — matching the ported prototype's design of not making guests hunt for the "right" spot to tap
- **Two animations fire together, not sequenced**: tapping either target sends the card flying straight up off-screen (`translateY(-115vh)`, 900ms, `cubic-bezier(0.22, 1, 0.36, 1)` — an eased-out "flung" motion) while the entire scene (crest, tagline, envelope, hint) fades and scales down slightly (`opacity: 0`, `scale(0.94)`, 700ms, starting 250ms after the tap) — the short delay means the card is already mid-flight before the rest of the scene starts to dissolve behind it
- **Timing is a config item** at the top of `EnvelopeIntro.jsx`: `CARD_FLY_DURATION` (900ms), `CONTENT_FADE_DURATION` (700ms), `CONTENT_FADE_DELAY` (250ms), `HINT_DELAY` (3200ms). The matching CSS `transition` durations on `.envelope-box__card` and `.envelope-intro__content` must be kept in sync with these by hand — there's no shared config file
- Degrades to an instant, motion-free transition under `prefers-reduced-motion` (card/content transitions removed, hint appears immediately without its bounce animation)
