...
| Element | Size |
|---|---|
| `h1` (Blessings Wall page title only — Hero no longer has one, see Hero below) | `clamp(2.5rem, 6vw, 4rem)` — scales with viewport, min 40px, max 64px |
...
- Structure, all siblings inside `.envelope-intro__content`: the **Ganesh crest** (`ganeshCutoutWithoutBg.png`, 165px) → the tagline as an overline → the **envelope box** described above (nudged down from the crest with its own `margin-top`, rather than sitting flush) with the couple's monogram seal at the fold and a card tucked inside, invisible until opened → a two-line Sanskrit invocation — the Devanagari "‖ श्री गणेशाय नमः ‖" (`.envelope-intro__invocation--sanskrit`, Hero's own shloka font, no uppercase transform or heavy letter-spacing — both meaningless/harmful on this script) directly above its English transliteration ("‖ Shree Ganeshay Namah ‖") — → a "tap the seal to open your invitation" hint (italic, no icon)
...
### Hero
- **Two full-screen views stacked inside one `<section id="hero">`**, not a single centered block — `.hero__view--intro` then `.hero__view--invite`, each its own `min-height: 100svh` flex-centered box (`.hero__view`). Moving from one to the other is a plain scroll past that boundary, not a slideshow/snap transition, so it composes for free with Nav's own scroll listener and the up/down section frame-nav. `.hero` itself is now just a plain positioned wrapper (background-color only); the per-screen sizing/centering that used to live on `.hero` moved onto `.hero__view`
- **No background photo** — a plain `--color-bg` (ivory) background; `content.hero.backgroundImage` and the placeholder image file were removed entirely (nothing references them anymore)
- **Glitter effect**, scoped to the intro view only (`.hero__view--intro`, which is also the only view with `overflow: hidden`, since both effects are absolutely positioned relative to it and would otherwise span the full stacked height of both screens): ~16 small soft sparkles scattered across the view, each twinkling on its own staggered timer (fade + scale via `@keyframes sparkle-twinkle`), plus a slow diagonal light-shimmer band that sweeps across it on a ~9s loop. Both are `pointer-events: none` (decorative only) and disabled under `prefers-reduced-motion`. Sparkle color is warm gold (`rgba(176, 137, 104, ...)`) rather than near-white — against the flat ivory background a near-white dot barely registered; this was originally tuned for contrast against a busy photo that no longer exists
- **First screen (`.hero__view--intro`) — the invocation alone**, nothing competing for attention: crest, two shlokas each with its own English translation, and a small ornamental divider between them. Top to bottom:
  1. **Ganesh crest** (`.hero__ganesh`, 200px — the same photographic asset as the Envelope Intro's own crest, `ganeshWithoutBackground.png`, see that section's Assets note; replacing an earlier hand-built inline SVG, `GaneshArt.jsx`, since removed) with a pulsing golden-pink radial glow behind it (`.hero__ganesh-glow`, `@keyframes hero-ganesh-glow-pulse`, ~5.5s, breathing opacity + scale — an earlier version cross-faded the gradient's own colors between gold- and pink-leaning stops, which read as muddy rather than a clean glow, so it was switched to one fixed blended color that just pulses instead, the same technique as the Envelope Intro seal's own glow). **No `z-index` on the glow** — it's deliberately first in DOM order among `.hero__ganesh`'s children, which alone puts it behind the crest image at `z-index: auto`; an earlier `z-index: -1` here escaped `.hero__ganesh` entirely (`position: relative` alone doesn't create a stacking context without an explicit `z-index`) and sank the glow all the way to the document root, hiding it behind the page's own background with zero visual indication why — a real, previously-undetected bug, not just a style choice
  2. **Sanskrit shloka** for Ganesh (the mantra "वक्रतुण्ड महाकाय सूर्यकोटिसमप्रभ । निर्विघ्नं कुरु मे देव सर्वकार्येषु सर्वदा ॥") plus its **English translation** (`.hero__shlok-translation`, italic, muted, narrower max-width) directly below
  3. **`.hero__shlok-divider`** — a small ornamental rule (a thin gold line fading at both ends, `linear-gradient`, with a centered dot via `::before`) marking the boundary between the two shlokas
  4. A second Sanskrit shloka, the Vishnu maṅgalam verse ("मंगलम् भगवान विष्णुः, मंगलम् गरुणध्वजः । मंगलम् पुण्डरी काक्षः, मंगलाय तनो हरिः ॥"), plus its own English translation, same treatment as the first
  - Both shlokas are hardcoded (not in `content.js` — fixed devotional text, not couple-specific data, same precedent as other static phrases already inline in Hero.jsx), and both are rendered via **`FitDevanagari`** (`src/components/FitDevanagari.jsx`), not a plain `<p>`: it measures each Devanagari half-line's natural (unwrapped) width against the available container width on mount and on `ResizeObserver` resize, and solves directly for the exact font-size at which the widest line fits — one measurement pass, no iterative shrink loop, since natural width scales linearly with font-size so the fit is exact on the first try regardless of the size currently applied. This guarantees each shloka stays on exactly two lines (one per verse-half) at *any* viewport width, rather than wrapping onto three or four once a fixed CSS `clamp()` min-font-size floor was hit on narrow phones (the previous approach, and the bug this replaced) — the English translation underneath is deliberately left free to wrap onto as many lines as it needs instead, as the pressure valve that absorbs whatever width is left over
- **Second screen (`.hero__view--invite`) — the actual invitation**, top to bottom:
  1. Eyebrow tagline (`hero.tagline`)
  2. **Invitation paragraph** (`.hero__invite`): "We cordially invite you on the auspicious union of", then the bride's parents (`content.coupleProfiles.bride.parentage`, mother listed first), the couple's first names (`.hero__invite-name`, via `content.couple.partner1/2` — not the profiles' full surnamed names) separated by an "&", then the groom's parents (mother first, same convention) — no separate `<h1>` names heading above this; the couple's names live only here now
  3. The scratch-reveal card (date + countdown, see below)
- Max content width 40rem (`.hero__overlay`, present once per view), centered
