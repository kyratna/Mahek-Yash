...
- **Design/animation reference**: modeled on a dusty-rose envelope-opening reel (a competitor product's demo, `adorableinviteswebsites1.netlify.app`), rebuilt from scratch in this project's plain JS/CSS — no new dependencies, no code or assets copied. The 3-beat choreography (open → pause → fly out) is this site's own; only the envelope's color tone (dusty rose body/flap on a warm near-black backdrop) and the card's slight tilt-then-straighten rise were carried over
- **Dusty-rose envelope, no photo asset**: unlike the rest of the site (which dropped its one background photo, see Hero above), this scene is intentionally the one dark, saturated moment on the whole site — the body uses a rose gradient (`--envelope-rose-1/2`) sitting on a near-black warm backdrop (`--envelope-backdrop-1/2`), with the same inline SVG `feTurbulence` grain trick used elsewhere on the site for paper texture (`--envelope-noise`) standing in for a photographed backdrop. `--envelope-flap-fill` is set to `--envelope-rose-1` (the body's own top-edge tone), not a separate color — a mismatched flap color was the first thing that made this read as two disconnected shapes instead of one closed envelope; the flap's own depth/fold comes entirely from the dark shading gradient layered on top of it in the SVG, not from a different base fill
- **Flap and body meet edge-to-edge**, not overlapping by half — both are sized off a single `--flap-apex: 46%` custom property on `.envelope-box` (flap height, and the body's height/seal position are all derived from it), so the seal sits exactly at the point where the flap's tip meets the body, like a real wax seal, and retuning the split only means changing that one value
- Structure, all siblings inside `.envelope-intro__content`: a small **Ganesh crest** (shared `GaneshArt.jsx`, same artwork as Hero and the favicon) → the tagline as an overline → the **envelope box** (`.envelope-box`: a rose gradient body, an SVG triangle flap on top, a circular gold monogram seal — e.g. "M & Y", from `content.couple` — sitting at the flap's apex, and an ivory card tucked inside, invisible until opened) → a "tap to open" hint with a bouncing arrow that fades in after a 3.2s pause (`HINT_DELAY`), so it doesn't compete with the initial view
- **Card copy matches the reference reel's structure** (see above), values swapped for this couple's real data: an eyebrow ("The Wedding Of") → names ("Mahek & Yash", from `content.couple`) → a thin divider rule → the wedding date (`content.wedding.displayDate`) → the venue (`content.mapAddress`, uppercase/tracked)
- **One tap target — the seal — drives a 3-beat sequence**, each beat timed by its own `setTimeout` in `handleOpen`, deliberately unhurried so it reads as a hand actually opening an envelope rather than a UI transition:
  1. **Open**: the flap starts tilting back immediately (`rotateX(-150deg)`, 900ms, `.envelope-box__flap--open`); the card waits a beat (`CARD_RISE_DELAY`, 300ms, a CSS `transition-delay` on `.envelope-box__card`) before it lifts out at a slight angle and straightens as it settles (`translateY` + `scale` + `rotate(-7deg → 0)`, 900ms, `.envelope-box__card--risen`) — the stagger is what sells "pulled out from behind the flap" instead of both pieces moving in lockstep
  2. **Pause** (`PAUSE_DURATION`, 1000ms): the card just sits there, fully settled, so it's actually readable before anything else happens
  3. **Flying** (`phase: "flying"`, `.envelope-box__card--flying`): the card rushes toward the viewer — a big scale-up plus fade (`scale(7)`, `opacity: 0`, the standard fake-dolly-zoom trick for a flat 2D element) — while the rest of the scene (crest, tagline, envelope, hint) fades and scales down slightly behind it (`.envelope-intro__content--flying`), together reading as the card flying out through the screen as the whole envelope dissolves
- **Timing is a config item** at the top of `EnvelopeIntro.jsx`: `FLAP_OPEN_DURATION` (900ms), `CARD_RISE_DELAY` (300ms), `CARD_RISE_DURATION` (900ms), `PAUSE_DURATION` (1000ms), `CARD_FLY_DURATION` (750ms), `SCENE_FADE_DURATION` (700ms), `HINT_DELAY` (3200ms). The matching CSS `transition` durations/delays on `.envelope-box__flap`, `.envelope-box__card`/`.envelope-box__card--flying`, and `.envelope-intro__content` must be kept in sync with these by hand — there's no shared config file
- Degrades to an instant, motion-free transition under `prefers-reduced-motion` (flap/card/content transitions removed, hint appears immediately without its bounce animation)
