...
- **Deep-maroon envelope, no photo asset**: unlike the rest of the site (which dropped its one background photo, see Hero above), this scene is intentionally the one dark, saturated moment on the whole site — the body uses a maroon gradient (`--envelope-maroon-1/2`) sitting on a near-black warm backdrop (`--envelope-backdrop-1/2`), with the same inline SVG `feTurbulence` grain trick used elsewhere on the site for paper texture (`--envelope-noise`) standing in for a photographed backdrop

#### Assets: commissioned art, and why it's re-exported before use
The couple's monogram (an "MY" wordmark with a laurel-wreath ring) and the Ganesh crest used here are AI-generated source images dropped into `public/images/monogram/` and `public/images/lordganesh/` — both **4096px, several MB, and with no real alpha channel**: the generator "fakes" transparency by painting a literal checkerboard pattern into the opaque RGB data instead of an actual alpha channel (confirmed via `sips -g hasAlpha` → `no`). Used as-is, that checkerboard renders as real pixels.

Naive fixes don't fully work here: a flood-fill keyed off one reference gray only catches pixels *connected to the border*, leaving small unreached islands next to fine linework (the wreath, letter serifs); the checkerboard itself turned out to be a genuine two-tone grid (white **and** gray, not one flat tone), so a single-color threshold misses half of it; and even a correct cutout can still fringe at PNG-resize time, because Pillow's resize isn't alpha-premultiplied — "removed" pixels keep their old checkerboard RGB under `alpha=0`, and a resize kernel spanning a removed and a kept pixel blends that stale RGB into the new edge pixel's color. What actually worked: `rembg` (`isnet-general-use` ONNX model, semantic segmentation instead of color-matching) for the cutout, and — for every place this art sits inside a CSS 3D transform (the flap's `rotateX`, the card's `rotateY` flip) — pre-flattening the art onto that exact spot's own solid backing color at export time (`monogramseal-on-blush.png` onto `--seal-fill`, `monogramseal-on-wine.png` onto `--card-back-fill`) rather than shipping a transparent PNG, so there's no alpha-compositing left for the browser to do mid-transform at all. The Ganesh crest and the nav-bar logo aren't inside any 3D transform, so they ship as real transparent PNGs (`lordganesh-clean-220.png`, `monogramseal-clean-180.png`). All derivatives are pre-sized (not full 4096px) and referenced through the `asset()` helper in `content.js` so they resolve under Vite's `base` path.

- **Flap and body read as one solid rounded card, not two stacked shapes, and the card is only ever visible through the flap's own triangular silhouette** — not the flap's full rectangular bounding box. Structure: a static `.envelope-box__flap-backing` (never animates, same fill as the body) sits full-width under everything, so the top band is always envelope-colored no matter what; on top of it, `.envelope-box__flap` is a separate `<div>` that owns the open animation and contains an actual `<polygon points="0,0 288,0 144,130">` triangle (flat top edge, point at the envelope's vertical center) plus a stroked V crease line, rather than a rectangle — a rectangle here was tried and rejected: it let the card become visible the instant the flap started lifting, regardless of shape, instead of peeking through a point that widens as the flap opens, which is what actually reads as "envelope" rather than "lid." A raw `<polygon>` has no rounded corners of its own, though — `.envelope-box__flap-surface` (the div wrapping that SVG) carries its own `border-radius` + `overflow: hidden` so the triangle's sharp top-left/top-right points get clipped to match the backing's rounded corners, instead of poking a small "ear" above the envelope's smooth silhouette. `.envelope-box__flap-surface` also sets `backface-visibility: hidden` — without it, once the flap rotates past 90° open, the browser renders this surface's *mirrored back*, and the rounded-corner clip no longer lines up with the envelope beneath it, showing up as a stray notch at the top corners; hiding the back face entirely (rather than showing a garbled mirror image) reads as the flap cleanly disappearing from view once it's swung out of the way, which also happens to reveal the crest above it in full instead of leaving it partly covered. The fold depth (`--flap-apex`) is a classic banker-style **50%** — the crease point reaches the envelope's exact vertical center, where the seal sits directly on top of it, covering the tip
...
- **The seal lifts together with the flap instead of just vanishing**: the seal `<button>` is nested *inside* `.envelope-box__flap` (not a sibling positioned independently), anchored to the flap's own bottom edge (`top: 100%` of the flap box, not `--flap-apex` again — that would double-apply the envelope-relative percentage). Being a child, it inherits the flap's `rotateX` for free, and stays mounted (not conditionally unmounted) so it's there to animate — clicks past the first tap are already blocked by `.envelope-intro--open { pointer-events: none }` on the root, so no extra guard is needed. Left inheriting the flap's full rotation, though, the seal would end up heavily perspective-foreshortened at rest (viewed near edge-on) instead of a readable disc — `.envelope-box__flap--open .envelope-box__seal` counters that with its own `rotateX(-150deg)`, exactly canceling the parent's rotation so the seal settles back onto a flat, straight-on, right-reading plane once open, timed to match `FLAP_OPEN_DURATION` so the correction lands in step with the flap's own motion rather than snapping at the end
- Structure, all siblings inside `.envelope-intro__content`: the **Ganesh crest** (`lordganesh-clean-220.png`, 145px) → the tagline as an overline → the **envelope box** described above (nudged down from the crest with its own `margin-top`, rather than sitting flush) with the couple's monogram seal at the fold and a card tucked inside, invisible until opened → a Sanskrit invocation ("‖ Shree Ganeshay Namah ‖") → a "tap the seal to open your invitation" hint (italic, no icon)
- **The seal** (`.envelope-box__seal`) is a blush-colored disc (`--seal-fill: #f1dfd4`, chosen distinct from the site's ivory `--color-bg`) sized to sit proportionally at the fold rather than dominate the envelope (72px — a quarter of the envelope's width), ringed with a 3px border in `--card-back-fill` (a darker shade of the envelope itself, not a new one-off color — like a rim pressed into wax), with a soft gold glow radiating from its edge (`box-shadow`, using the site's own `--color-accent` rather than a one-off color), showing the couple's monogram artwork pre-flattened onto that same blush tone (see Assets above)
- **Card copy matches the reference photo/reel's structure**, values swapped for this couple's real data: an eyebrow ("The Wedding Of") → names ("Mahek & Yash", from `content.couple`) → a thin divider rule → the wedding date (`content.wedding.displayDate`) → the venue (`content.mapAddress`, uppercase/tracked). The front face's own background is `--seal-fill` (the seal's own blush tone), not the site's plain ivory `--color-bg`, so the card reads as part of the same envelope/seal palette rather than a mismatched paper-white insert
- **The card has two faces and flips between them** before flying out — not just a single face that flies away. `.envelope-box__card` is a plain position/transform shell (rise, fly) with `perspective` on it, sitting at `z-index: 5` — above the flap's `z-index: 3` at all times, not only once flying, since the flap's rotated silhouette otherwise pokes over the card's own top corners as it swings past. `.envelope-box__card-inner` is the actual flipping element (`transform-style: preserve-3d`, `rotateY(180deg)` when `.--flipped`), holding two absolutely-stacked `.envelope-box__card-face` children (`backface-visibility: hidden` on both, so only one is ever showing): `--face--front` is the existing eyebrow/names/date/venue card, `--face--back` is a darker, distinct surface (`--card-back-fill: #3a1420` — deliberately darker than both envelope maroons, not reused, so it reads as turning the card over to a different surface) showing the same monogram seal, pre-flattened onto that wine tone
- **One tap target — the seal — drives a 4-beat sequence**, each beat timed by its own `setTimeout` in `handleOpen`, deliberately unhurried so it reads as a hand actually opening an envelope rather than a UI transition:
  1. **Open**: the flap starts tilting up immediately (`rotateX(150deg)`, 900ms, `.envelope-box__flap--open`, seal riding along with it — and counter-rotating back upright, see above); the card waits a beat (`CARD_RISE_DELAY`, 300ms, a CSS `transition-delay` on `.envelope-box__card`) before it lifts out at a slight angle and straightens as it settles (`translateY` + `scale` + `rotate(-7deg → 0)`, 900ms, `.envelope-box__card--risen`) — the stagger is what sells "pulled out from behind the flap" instead of both pieces moving in lockstep
  2. **Pause** (`PAUSE_DURATION`, 1000ms): the card just sits there, front face showing, fully settled, so it's actually readable before anything else happens
  3. **Flipped** (`phase: "flipped"`, `CARD_FLIP_DURATION` 700ms + `BACK_PAUSE_DURATION` 500ms): `.envelope-box__card-inner` rotates 180° to the darker monogrammed back face, which then sits for a beat so the seal actually registers before flying
  4. **Flying** (`phase: "flying"`, `.envelope-box__card--flying`): the card — now showing its back face — rushes toward the viewer, a big scale-up (`scale(7)`) that stays fully opaque and jumps to `z-index: 10`, above every other layer in the scene — it reads as a solid card rushing into the foreground and covering everything behind it, not a ghost fading through them. The rest of the scene (crest, tagline, envelope, hint) still fades and scales down slightly underneath it (`.envelope-intro__content--flying`), a subtler dissolve happening behind the card's own opaque cover. This composes correctly because the fly-out transform lives on the *outer* `.envelope-box__card`, independent of the inner flip transform — whichever face the flip left showing is whatever flies
- **Timing is a config item** at the top of `EnvelopeIntro.jsx`: `FLAP_OPEN_DURATION` (900ms), `CARD_RISE_DELAY` (300ms), `CARD_RISE_DURATION` (900ms), `PAUSE_DURATION` (1000ms), `CARD_FLIP_DURATION` (700ms), `BACK_PAUSE_DURATION` (500ms), `CARD_FLY_DURATION` (750ms), `SCENE_FADE_DURATION` (700ms). The matching CSS `transition` durations/delays on `.envelope-box__flap`, `.envelope-box__card`/`.envelope-box__card--flying`, `.envelope-box__card-inner`, and `.envelope-intro__content` must be kept in sync with these by hand — there's no shared config file. The hint's own 3.2s reveal delay is *not* one of these JS constants — it's pure CSS (`animation-delay` on `.envelope-intro__hint`) — deliberately so: the hint is always mounted from first paint (reserving its layout space immediately) and purely fades in in place, rather than being mounted late by a `setTimeout` the way it originally was, which caused the whole scene to visibly reflow around it appearing mid-view
...
- The couple's monogram artwork sits in the bar as the logo (`monogramseal-clean-180.png`, transparent, same source as the Envelope Intro's seal — see its Assets note) — clicking/tapping it scrolls straight back to the Hero
...
- Content, top to bottom:
  1. **Ganesh vector art** (`.hero__ganesh`, `GaneshArt.jsx` — a hand-built inline SVG, not the photographic Ganesh crest used in the Envelope Intro or the monogram favicon, see that section's Assets note), small (64px) and centered, with a soft warm radial glow behind it — a stronger/more saturated glow recipe than the same technique elsewhere on the site, since a softer version nearly disappeared against the very light `--color-bg`
