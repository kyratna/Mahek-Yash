...
Three files, each used in exactly one shape/context, referenced through the `asset()` helper in `content.js` so they resolve under Vite's `base` path:
- `ganeshWithoutBackground.png` — the crest above the envelope, *and* Hero's own Ganesh mark (`.hero__ganesh-art`, replacing an earlier hand-built SVG icon — see Hero below)
...
- **Flap and body read as one solid rounded card, not two stacked shapes, and the card is only ever visible through the flap's own triangular silhouette** — not the flap's full rectangular bounding box. Structure: `.envelope-box__flap-backing` sits full-width under everything, so the top band is always envelope-colored no matter what; on top of it, `.envelope-box__flap` is a separate `<div>` that owns the open animation. Its actual triangle silhouette (flat top edge, point at the envelope's vertical center) is `.envelope-box__flap-fill`, a plain `<div>` shaped via `clip-path: polygon(0% 0%, 100% 0%, 50% 100%)`, sitting behind a small `<svg>` that draws only a soft shade-gradient overlay and a stroked V crease line on top of it — rather than a full rectangle: a rectangle here was tried and rejected, since it let the card become visible the instant the flap started lifting, regardless of shape, instead of peeking through a point that widens as the flap opens, which is what actually reads as "envelope" rather than "lid." Neither `clip-path` nor a raw `<polygon>` has rounded corners of its own, though — `.envelope-box__flap-surface` (the wrapper around both the fill div and the svg) carries its own `border-radius` + `overflow: hidden` so the triangle's sharp top-left/top-right points get clipped to match the backing's rounded corners, instead of poking a small "ear" above the envelope's smooth silhouette. `.envelope-box__flap-surface` also sets `backface-visibility: hidden` — without it, once the flap rotates past 90° open, the browser renders this surface's *mirrored back*, showing up as a stray gray-brown wedge either side of the seal instead of the flap cleanly vanishing. Getting that to actually take effect needs one more thing, easy to miss: `.envelope-box__flap` itself (the element that owns the `rotateX`) must set `transform-style: preserve-3d` — with the default `flat`, the rotation flattens `.envelope-box__flap-surface`'s content into the parent's plane *before* the child's own `backface-visibility` is evaluated, so past 90° the flattened/mirrored content keeps painting regardless of the `hidden` value; `preserve-3d` gives the child a genuine shared 3D context to actually hide itself in. `.envelope-box__flap-backing` mirrors the flap's own triangle exactly (same `clip-path` points, inside a rounded/clipped wrapper matching `.envelope-box__flap-surface`'s pattern) rather than staying a plain rectangle once open — its *inner* fill is a separate element (`.envelope-box__flap-backing-fill`) that clip-paths from a full rectangle down to that same triangle on the flap's own `FLAP_OPEN_DURATION` timing, so at rest (closed) it's a full rectangle (safely covering the flap's transparent side-wedges at every rotation angle, since the flap's own rotating bounding box is a full rectangle too — only its triangle is painted), and by the time the flap has visually vanished past 90° it's morphed down to the matching triangle, so no rectangular block is left behind it. The fold depth (`--flap-apex`) is a classic banker-style **50%** — the crease point reaches the envelope's exact vertical center, where the seal sits directly on top of it, covering the tip
- **Paper texture**: the body, the lid's own fill (`.envelope-box__flap-fill`), and the static backing underneath it (`.envelope-box__flap-backing-fill`) all use the *literal same* `--envelope-paper-grain` CSS `background-image` + `background-blend-mode: overlay` recipe — deliberately plain CSS, not an SVG `feTurbulence` filter, even though the lid's own fill has to be triangle-shaped (via `clip-path`, not an SVG polygon). An SVG-filter version was tried first: it looked visually *different* on each surface — sharper/finer on the lid's wide `288`-unit viewBox, coarser on the body's `100`-unit one — because `feTurbulence`'s `baseFrequency` is evaluated in each element's own local coordinate space, so the identical filter value produces a different apparent grain scale depending on how big that element's own viewBox numbers are. A plain tiled raster background has no per-element rescaling, so it's the only way to get all three surfaces to render *identical* texture instead of three techniques (or the same technique at three different implicit scales) drifting apart in shade — this is what "one seamless color" across the closed envelope actually depends on, not just matching hex values. The body also carries a thin decorative `<polyline>` V (`.envelope-box__body-seam-line`) tracing where a real diamond-flap envelope's tucked side flaps would seam, sitting just below the flap's own crease apex so the open envelope reads as folded paper rather than a flat rectangle. That polyline's own wrapping `<svg>` needs an explicit CSS `width: 100%` (not just `inset-inline: 0`) — an absolutely positioned SVG root with an intrinsic ratio computes an "auto" width from that ratio times its explicit height rather than stretching to fill its container, so without it the svg rendered as a square clipped to the left edge, leaving the right ~57% of the body with no seam-line or (when it still used one) grain at all
...
- **One tap target — the seal — drives a 4-beat sequence**, each beat timed by its own `setTimeout` in `handleOpen`, deliberately unhurried so it reads as a hand actually opening an envelope rather than a UI transition:
  ...
  2. **Pause** (`PAUSE_DURATION`, 2000ms): the card just sits there, front face showing, fully settled, so it's actually readable before anything else happens — lengthened from an earlier, brisker 1000ms so there's real time to read it
  3. **Flipped** (`phase: "flipped"`, `CARD_FLIP_DURATION` 700ms + `BACK_PAUSE_DURATION` 1000ms): `.envelope-box__card-inner` rotates 180° to the darker monogrammed back face, which then sits for a beat so the seal actually registers before flying — this pause was also lengthened, from an earlier 500ms
